...
int add_files_to_list(Options *opts, FileNode *current, glob_t *results);
int expand_wildcards_and_add(const char *pattern, Options *opts, FileNode *current);
...
void print_usage(char *errmsg)
{
    ...
    printf("  Commands: archive - Archive the listed files.\n");
    printf("  Archive mode options:\n");
    printf("    -p - password protected archive\n");
}
...
{
    // Read archive name
    opts->archive_name = strdup(argv[optind+1]); // Duplicate archive name
    ...
    for (int i = optind + 2; i < argc; i++)
    {
        ...
        {
            //Expand wildcards
            if (expand_wildcards_and_add(argv[i], opts, current) != 0)
            {
                perror("Could not handle wildcards");
                return 1;
            }
        }
        else
        {
            ...
            new_file->file_name = strdup(argv[i]); // Duplicate the file name
...
// Function to add files from glob results into linked list
int add_files_to_list(Options *opts, FileNode *current, glob_t *results)
{
    for (size_t i = 0; i < results->gl_pathc; i ++)
    {
        ...
        new_file->file_name = strdup(results->gl_pathv[i]); // Copy full path
        if (new_file->file_name == NULL)
        {
            perror("Could not allocate memory for file name");
            free(new_file);
...
int expand_wildcards_and_add(const char *pattern, Options *opts, FileNode *current)
{
    ...
    if (ret == 0)
    {
        if (add_files_to_list(opts, current, &results) != 0)
        {
            return 1;
        };
    }
